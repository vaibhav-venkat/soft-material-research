...
def ou_process_variance(
    dt: jax.Array | float,
    gamma: jax.Array | float,
    diffusion_u: jax.Array | float,
) -> jax.Array:
    """Exact innovation variance of a scalar continuous-time OU process."""
    return diffusion_u / gamma * (1.0 - jnp.exp(-2.0 * gamma * dt))


def integrated_ou_variance(
    dt: jax.Array | float,
    gamma: jax.Array | float,
    diffusion_u: jax.Array | float,
) -> jax.Array:
    """Exact stationary variance of ``∫u(t)dt``."""
    coefficient = (1.0 - jnp.exp(-gamma * dt)) / gamma
    return 2.0 * diffusion_u / gamma**2 * (dt - coefficient)


class OUIntervalMoments(NamedTuple):
    transition: jax.Array
    coefficient: jax.Array
    process_variance: jax.Array
    process_integral_covariance: jax.Array
    integral_process_variance: jax.Array


def ou_interval_moments(
    dt: jax.Array | float,
    gamma: jax.Array | float,
    diffusion_u: jax.Array | float,
) -> OUIntervalMoments:
    """Exact joint moments of the next OU state and its interval integral."""
    transition = jnp.exp(-gamma * dt)
    coefficient = (1.0 - transition) / gamma
    stationary_variance = diffusion_u / gamma
    return OUIntervalMoments(
        transition=transition,
        coefficient=coefficient,
        process_variance=ou_process_variance(dt, gamma, diffusion_u),
        process_integral_covariance=(
            stationary_variance * coefficient * (1.0 - transition)
        ),
        integral_process_variance=(
            integrated_ou_variance(dt, gamma, diffusion_u)
            - stationary_variance * coefficient**2
        ),
    )
...
def _kalman_filter(
    ...
    """Marginalize the colored rate and segment transfer with an exact scan."""
    gamma, _, diffusion_u, _, _, sigma_b = parameters
    mean = jnp.zeros((2, observations.shape[1]), dtype=jnp.float64)
    covariance = jnp.diag(jnp.stack((diffusion_u / gamma, sigma_b**2)))
    ...
    def step(
        ...
        moments = ou_interval_moments(step_dt, gamma, diffusion_u)
        transition_matrix = jnp.diag(jnp.stack((moments.transition, 1.0)))
        state_process_covariance = jnp.diag(
            jnp.stack((moments.process_variance, 0.0))
        )
        measurement = jnp.stack((moments.coefficient, step_dt))
        state_measurement_covariance = jnp.stack(
            (moments.process_integral_covariance, 0.0)
        )
        (
            updated_mean,
            updated_covariance,
...
def _kalman_coefficients_single(
    ...
    gamma, _, diffusion_u, _, _, _ = parameters
    mean = jnp.zeros((1, dimension), dtype=jnp.float64)
    sensitivity = jnp.zeros((1, dimension), dtype=jnp.float64)
    covariance = jnp.reshape(diffusion_u / gamma, (1, 1))
    ...
    def step(
        ...
        moments = ou_interval_moments(step_dt, gamma, diffusion_u)
        transition_matrix = jnp.reshape(moments.transition, (1, 1))
        integral_coefficients = jnp.reshape(moments.coefficient, (1,))
        integral_process_covariance = jnp.reshape(
            moments.process_integral_covariance, (1,)
        )
        integral_process_variance = moments.integral_process_variance
        ...
        propagated_covariance = (
            ...
            @ previous_covariance
            @ transition_matrix.T
            + jnp.reshape(moments.process_variance, (1, 1))
        )
        cross_covariance = (
...
def transition(
    ...
    gamma, kappa_total, diffusion_u, diffusion_total, area_star, sigma_b = parameters
    total = jnp.sum(area)
    ...
    total_variance = ou_process_variance(dt, kappa_total, diffusion_total)
...
def parameter_negative_log_likelihood(
    ...
    _, kappa_total, _, diffusion_total, area_star, sigma_b = parameters
    ...
    for block in data.blocks:
        ...
        variance = (
            ou_process_variance(block.dt, kappa_total, diffusion_total)
            + JITTER
        )
        log_density = -0.5 * (
